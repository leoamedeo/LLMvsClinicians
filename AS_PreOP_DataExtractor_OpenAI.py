...
output_folder = "./output_files"   # Path to save output Excel files

model_name = "o3-mini" #select desired OpenAI model here
llm_nickname = "o3mini"
...
input_instructions = (
    "You are a helpful physician assistant tasked with extracting clinical data for a study. You will be provided with a document, more specifically a medical record. Your task is to look look for the following preoperative symptoms: Sudden Severe Facial Pain, Facial Numbness, Vertigo, Lacrimation, Facial Muscle Spasm, and Other (related to trigeminal neuralgia)."
        ...
        if file_name.endswith(".docx"):
            ...
            try:
                with open(file_path, "rb") as docx_file:
                    ...
                    # Extract text between "diagnos" and "freundlich" (case-insensitive), including delimiters
...
def standardize_variable_name(variable):
    """Normalize different spellings for the same preoperative symptom."""
...
def process_and_run_llm_for_subfolder(subfolder_path, api_key, model=model_name):
    """Process documents in a subfolder and run LLM analysis."""
...
# Main execution loop
# Run the LLM system for each subfolder multiple times
